 ...
 * @see com.liferay.oauth.service.persistence.OAuthApplicationUtil
 * @generated
 */
@ProviderType
public class OAuthApplicationPersistenceImpl extends BasePersistenceImpl<OAuthApplication>
	...
	public OAuthApplication fetchByConsumerKey(String consumerKey,
		...
		if (retrieveFromCache) {
			result = finderCache.getResult(FINDER_PATH_FETCH_BY_CONSUMERKEY,
					finderArgs, this);
		}
		...
		if (result == null) {
			...
			try {
				...
				finderCache.removeResult(FINDER_PATH_FETCH_BY_CONSUMERKEY,
					finderArgs);
	...
	protected void cacheUniqueFindersCache(
		OAuthApplicationModelImpl oAuthApplicationModelImpl, boolean isNew) {
		if (isNew) {
			Object[] args = new Object[] {
					oAuthApplicationModelImpl.getConsumerKey()
				};

			finderCache.putResult(FINDER_PATH_COUNT_BY_CONSUMERKEY, args,
				Long.valueOf(1));
			finderCache.putResult(FINDER_PATH_FETCH_BY_CONSUMERKEY, args,
				oAuthApplicationModelImpl);
		}
		else {
	...
	protected void clearUniqueFindersCache(
		...
		finderCache.removeResult(FINDER_PATH_COUNT_BY_CONSUMERKEY, args);
		finderCache.removeResult(FINDER_PATH_FETCH_BY_CONSUMERKEY, args);
	...
	public OAuthApplication create(long oAuthApplicationId) {
		...
		oAuthApplication.setCompanyId(companyProvider.getCompanyId());
	...
	public OAuthApplication updateImpl(OAuthApplication oAuthApplication) {
		...
		clearUniqueFindersCache(oAuthApplicationModelImpl);
		cacheUniqueFindersCache(oAuthApplicationModelImpl, isNew);
	...
	/**
	 * Returns the o auth application with the primary key or throws a {@link com.liferay.portal.kernel.exception.NoSuchModelException} if it could not be found.
	 *
	 * @param primaryKey the primary key of the o auth application
