		...
		Liferay.Report = {
			...
			_addParameter: function(namespace) {
				...
				if (parameterKey.length == 0) {
					...
					message = Liferay.Language.get('please-enter-a-valid-report-parameter-key');
				...
				if (parameterType != 'date' && parameterValue.length == 0) {
					...
					message = Liferay.Language.get('please-enter-a-valid-report-parameter-value');
				...
				if (parameterKey.indexOf(',') > 0 || parameterKey.indexOf('=') > 0 || parameterValue.indexOf('=') > 0) {
					message = Liferay.Language.get('one-of-your-fields-contains-invalid-characters');
				...
				if (reportParameters) {
					...
					for (var i in reportParametersJSON) {
						...
						if (reportParameter.key == parameterKey) {
							message = Liferay.Language.get('that-vocabulary-already-exists');
			...
			_addTag: function(parameterKey, parameterValue, parameterType) {
				...
				var key = AEscape.html(('report-tag-' + parameterKey).replace(/ /g, 'BLANK'));
			...
			_sendMessage: function(message) {
				...
				message = AEscape.unescapeHTML(message);
